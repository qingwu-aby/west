...
class ViewsController extends Controller {
	 ...
	 * 获取单条剧集内容
	 */
	public function getEpisodesContent() {
		
		$uid = I('get.uid');
		$id = I('get.id');
		$Episodes = M('Episodes');
		$result = $Episodes->where(array('uid' => $uid, 'id' => $id))->find();
		if($result == null) {
			$data = array(
				'code' => -1,
				'msg' => '该数据不存在',
			...
			// 增加访问量
			$update = $Episodes->where(array('uid' => $uid, 'id' => $id))->data(array('views' => $result['views'] + 1))->save();
